    ...
    : BaseLlmProvider(httpClient)
{
        ...
        bool allowToolChoiceFallback)
    {
        ...
        for (var streamAttempt = 0; streamAttempt <= AppDefaults.MaxProviderRetries; streamAttempt++)
        {
            ...
            aggregateRaw.AppendLine($"# attempt {streamAttempt + 1}");
            ...
            if (streamAttempt >= AppDefaults.MaxProviderRetries || ct.IsCancellationRequested)
                break;
            ...
            retryRequest.Headers.Authorization = new AuthenticationHeaderValue("Bearer", config.ApiKey);
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                if (request.OnResponseJson is not null) await request.OnResponseJson(aggregateRaw.ToString());
                throw new LlmProviderException(
                    $"Cerebras API antwortete mit HTTP {(int)response.StatusCode}\n{body}");
            }
        }

        if (request.OnResponseJson is not null) await request.OnResponseJson(aggregateRaw.ToString());
        throw new LlmProviderException(
            $"Cerebras stream ended before [DONE] after {AppDefaults.MaxProviderRetries + 1} attempts." +
