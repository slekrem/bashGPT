...
public sealed class GhPrChecksTool : ITool
{
    private readonly IGhPolicy _policy;

    public GhPrChecksTool(IGhPolicy? policy = null)
    {
    ...
    public ToolDefinition Definition { get; } = new(
        Name: "gh_pr_checks",
        Description: "Returns the CI check results for a GitHub pull request. If no number is given, uses the PR for the current branch. Requires gh CLI to be installed and authenticated.",
        Parameters:
        [
    ...
    public async Task<ToolResult> ExecuteAsync(ToolCall call, CancellationToken ct)
    {
        ...
        var args = new List<string> { "pr", "checks" };
        if (number.HasValue) args.Add(number.Value.ToString());
        args.AddRange(["--json", "name,state,conclusion,link"]);

        var (stdout, stderr, exit) = await GhRunner.RunAsync(args, repoPath, ct);

        if (exit != 0)
            return new ToolResult(Success: false, Content: $"gh pr checks failed: {stderr.Trim()}");
