...
public sealed class GhPrDiffTool : ITool
{
    private readonly IGhPolicy _policy;

    public GhPrDiffTool(IGhPolicy? policy = null)
    {
    ...
    public ToolDefinition Definition { get; } = new(
        Name: "gh_pr_diff",
        Description: "Returns the diff of a GitHub pull request. If no number is given, uses the PR for the current branch. Requires gh CLI to be installed and authenticated.",
        Parameters:
        [
    ...
    public async Task<ToolResult> ExecuteAsync(ToolCall call, CancellationToken ct)
    {
        ...
        var args = new List<string> { "pr", "diff" };
        if (number.HasValue) args.Add(number.Value.ToString());

        var (stdout, stderr, exit) = await GhRunner.RunAsync(args, repoPath, ct);

        if (exit != 0)
            return new ToolResult(Success: false, Content: $"gh pr diff failed: {stderr.Trim()}");

        if (string.IsNullOrWhiteSpace(stdout))
            return new ToolResult(Success: true, Content: "No diff — the PR has no changes.");
